...
def everything_for_your_cat(cats_data):
    """
    Приводит информацию из списка кортежей к виду:'владелец: его питомцы' и выводит ее в виде строки
    :param cats_data: список кортежей, содержащих информацию о питомцах и их владельцах
    :return: строка вида 'владелец: питомец_1 .... питомец_n'
    """
    users = {}
    our_str = ''
    for tuple_item in cats_data:

        owner = f'{tuple_item[2]} {tuple_item[3]}'
        animal = f'{tuple_item[0]}, {tuple_item[1]}'

        if owner not in users:
            users.update({owner: animal})
        else:
            users[owner] += f'; {animal}'

    for owner, animal in users.items():
        our_str += f'{owner}: {animal}\n'
    return our_str
