...
def repeats(our_str):
    """
    Выставляет с каждым элементом строки номер его вхождения
    :param our_str: Строка, которую преобразовываем
    :return: Строка, в которой рядом с каждым элементом отображается его номер вхождения в строку
    """
    new_str = ''
    for item in range(len(our_str)):
        new_str += f'{our_str[item]}_{our_str[:item + 1].count(our_str[item])}'
    return new_str
