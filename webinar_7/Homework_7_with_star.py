...
class RomanNums:
    ...
    @staticmethod
    def roman_figure(num):
        """
        Преобразует римскую ЦИФРУ в арабскую
        :param num: Строка. Римское число
        :return: Число. Арабская цифра, соответствующая римской
        """
    ...
    def from_roman(self):
        ...
        :return: Число. Римское число в виде арабского
        """
        first_number = 0
        second_number = 0

        for item in self.num[::-1]:
            if self.roman_figure(item) >= second_number:
                first_number, second_number = first_number + self.roman_figure(item), self.roman_figure(item)
            else:
                first_number -= self.roman_figure(item)
    ...
    def is_palindrome(self):
        ...
        :return: Булевое значение: True если число палиндром, иначе False
        """
