...
class PersonInfo:
    def __init__(self, name, age, *departament):
        self.fio = name.split()
        self.age = age
        self.departament = [item for item in departament]

    def short_name(self):
        """
        Форматирует Имя Фамилию
        :return: Строка вида: Фамилия И.
        """
        short_name = f'{self.fio[1]} {self.fio[0][0]}.'
        return short_name

    def path_deps(self):
        """
        Выводит путь до подразделения
        :return: Строка. Путь до подразделения
        """
        path_deps = " --> ".join(self.departament)
        return path_deps

    def new_salary(self):
        """
        Высчитывает индексирование зарплаты
        :return: Число, на которое проиндексирована зарплата
        """
        magic_index = 0
        magic_dict = []
        magic_str = ''.join(self.departament)
        for item in set(magic_str):
            magic_dict.append(magic_str.count(item))

        for _ in range(3):
            magic_index += max(magic_dict)
            magic_dict.remove(max(magic_dict))

        new_salary = 1337 * self.age * magic_index
        return new_salary
